...
function flexibleGrade(input: string, keywords: string[]): boolean {
  ...
  if (keywords.length === 0) return norm.split(/\s+/).length >= 3;
  const matchCount = keywords.filter((kw) => norm.includes(kw.toLowerCase())).length;
  return matchCount >= Math.max(1, Math.ceil(keywords.length * 0.25));
}
              ...
              <div className="grid grid-cols-1 gap-3">
                {activity.options.map((option, i) => (
                  <Button
                    key={i}
                    variant="outline"
                    className={`justify-center text-center h-auto ${isK2 ? "py-5 px-5 text-2xl min-h-[64px]" : "py-3 px-4"} text-foreground hover:bg-primary/10 hover:border-primary/30`}
        ...
        {speech.isSupported ? (
          <>
            ...
            {answer && !speech.isListening && !nudgeMessage && (
              <div className="w-full bg-muted/50 rounded-lg p-4 border border-border space-y-2">
                ...
                <button
                  ...
                  className="text-base text-muted-foreground hover:text-destructive underline transition-colors"
                >
                  🔄 Try again
                </button>
              </div>
